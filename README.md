...
The data utilized for backtesting purposes is sourced from **iFind**, encompassing the close price data for the SPX500 index, as well as the open and close prices for all individual stocks within the S&P500 index[^3]. The portfolio weights, as previously mentioned, are generated through our strategy[^4]. To ensure data integrity, missing values are replaced with `NaN`, and the index is configured to `datetime` format. Our backtesting approach involves testing our strategy with weights generated by targeting daily returns of 1.5%, 2%, and 2.5% using the [Bayesian approach](#integration-with-bayesian-shrinkage-on-factors).

[^3]: The price data can be downloaded from the [GitHub repository](https://github.com/Gavin-OP/fina4380-project/tree/main/data).
[^4]: The weights are available for download from the [GitHub repository](https://github.com/Gavin-OP/fina4380-project/tree/main/output).
